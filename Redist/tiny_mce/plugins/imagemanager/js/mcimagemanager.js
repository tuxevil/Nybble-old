var mcImageManager = {
	...
	open : function(form_name, element_names, file_url, js, s) {
		...
		if (s.iframe) {
			...
			win.contentWindow.document.location = this.getBaseURL() + '/../default.aspx?type=im';
			return;
		} else
			win = window.open(this.getBaseURL() + '/../default.aspx?type=im' + pa, 'mcImageManagerWin', 'left=' + x + ',top=' + y + ',width=' + w + ',height=' + h + ',scrollbars=yes,resizable=yes,statusbar=no');
